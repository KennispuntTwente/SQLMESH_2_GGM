...
def run_sqlmesh(gateway: str, auto_apply: bool, dry_run: bool, verbose: bool) -> int:
    ...
    cmd = ["sqlmesh", "-p", "sqlmesh", "--gateway", gateway, "plan"]
    if auto_apply:
...
Examples:
    ...
    parser.add_argument(
        ...
        metavar="DEST",
        help=f"dlt destination (required). "
             f"Choices: {', '.join(DLT_DESTINATIONS)}",
    )
    parser.add_argument(
        "-g", "--gateway",
        default=None,
    ...
    gateway = args.gateway or get_env("GGM_GATEWAY", None) or _detect_gateway(destination)
    ...
    print(f"  Query data:   uv run sqlmesh -p sqlmesh --gateway {gateway} fetchdf \"SELECT * FROM silver.client LIMIT 10\"")
    print()
